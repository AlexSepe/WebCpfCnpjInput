...
import { CpfCnpjComp, CpfCnpjCompProps } from "../CpfCnpjComp";

describe("Badge", () => {
    const createBadge = (props: CpfCnpjCompProps): ShallowWrapper => shallow(<CpfCnpjComp {...props} />);

    it("should render the structure", () => {
        const badgeProps: CpfCnpjCompProps = {
            value: "0"
        };
        const badge = createBadge(badgeProps);

        expect(badge.equals(<span className="widget-webcpfcnpjinput badge label-default">0</span>)).toEqual(true);
    });

    it("should show 72676689000180 ", () => {
        const value = "72676689000180";
